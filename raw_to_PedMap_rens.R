...
dataset <- "NE100_100cm_rep2"
rep = "2"

dir.file <- paste0("Dataset/",dataset)
...
# load the pedigree 
animalsRep1_res <- read_table( paste0(dir.file,"/animalsRep",rep,".res.bz2"), 
                               col_types = cols(id = col_character(), 
                                                sire = col_character(), dam = col_character(), 
                                                X11 = col_skip()))

# Time 1: animals used for most of the plots and tables 
time_1 <- c(20,40,60,80,100)

# Time 2: animals used for rate of inbreedng 
time_2 <- c(12:17,18:22,32:37,38:42,52:57,58:62,72:77,78:82,89:100)


# filter relevant indiciduals  
rel_ID_1 <- animalsRep1_res %>%
  filter(birth %in% time_1)

rel_ID_2 <- animalsRep1_res %>%
  filter(birth %in% time_2)

# write out the ped of only the Time 1 individuals
write.table(rel_ID_1, file = paste(dir.file,"/pedigree_rel.generations.txt",sep=""), row.names = FALSE, quote = FALSE, sep = " " )
...
MarkerQtl <- read_table(paste0(dir.file,"/MarkerQtlGenotypesRep",rep,".res.bz2"), 
                        col_names = FALSE, 
...
write_csv(MarkerQtl_rel,paste0(dir.file,"/MarkerQtlGenotypesRep_rel.res.bz2"), quote = c("none") )  
...
markerPositionsRep1 <- read_table(paste0(dir.file,"/markerPositionsRep",rep,".res.bz2",sep = "") ) %>% 
  relocate(chromosome)
...
write.table(data_pos, file = paste0(dir.file,"/full.map"),
            row.names = FALSE, col.names = FALSE, sep = " ", quote = FALSE )


write.table(data_pos_50, file =  paste0(dir.file,"/full_50.map"), row.names = FALSE, col.names = FALSE, sep = " ", quote = FALSE )

write_csv(data_pos_50,file =  paste0(dir.file,"/50_markerPositionsRep",".res.bz2"), quote = c("none")  )
...
# SNP id and observable position is not the same 
# so i make a relative position column
...
#info_rel_gen <- unite(info_rel_gen, info_rel_gen, c(1:6), sep = " ")

#genome length before filtering
str_length(MarkerQtl_rel$Geno[1])

all_SNP <- c(1:(str_length(MarkerQtl_rel$Geno[1])/2))
#Find all markers without positional data
leftovers <- all_SNP[-data_pos$locus]

# positions without positionaldata 
...
# loop over all individuals and remove the markers that 
# don't have positional data
...
nchar(MarkerQtl_rel$Geno[1]) # n SNPs before
nchar(data$Geno[1])          # n Snps after
...
write.table(info_max, file = paste0(dir.file,"/full_gen100.ped"),row.names = FALSE, col.names = FALSE,sep = " ",quote = FALSE )
...
nchar(MarkerQtl_rel$Geno[1])
nchar(data_rel$Geno[1])
...
write.table(info_rel_gen, file=paste0(dir.file,"/full_rel_gen.ped"),row.names = FALSE, col.names = FALSE,sep = " ",quote = FALSE )
...
system(paste0("plink --map ",dir.file,"/full.map --ped ",dir.file,"/full_gen100.ped --maf 0.05 --make-bed --recode --out ",dir.file,"/gen_100_maf"))
...
map_maf <- read_table(paste0(dir.file,"/gen_100_maf.map"), 
                      col_names = FALSE)
...
# write out
write.table(train[,1:4], file = paste0(dir.file,"/train.map"), row.names = FALSE, col.names = FALSE, sep = " ", quote = FALSE )
write.table(train, file = paste0(dir.file,"/train_temp.map"), row.names = FALSE, col.names = FALSE, sep = " ", quote = FALSE )
write.table(test, file = paste0(dir.file,"/test.map"), row.names = FALSE, col.names = FALSE, sep = " ", quote = FALSE )
...
write.table(train_50[,1:4], file = paste0(dir.file,"/train_50.map"), row.names = FALSE, col.names = FALSE, sep = " ", quote = FALSE )
write.table(train_50, file = paste0(dir.file,"/train_50_tmp.map", sep = ""), row.names = FALSE, col.names = FALSE, sep = " ", quote = FALSE )
write.table(test_50, file = paste0(dir.file,"/train_50_test.map", sep = ""), row.names = FALSE, col.names = FALSE, sep = " ", quote = FALSE )
...
# now with a extra layer of subseting aka the 50k 

# Here we use the new marker pos label (tmp_pos) since it looks at the position
...
write_csv(data_50k, file = paste0(dir.file,"/50_MarkerQtlGenotypesRep1.res.bz2") , quote = c("none")  )
...
# write it all out 
write.table(info_full, file= paste0(dir.file,"/full.ped"), row.names = FALSE, col.names = FALSE,sep = " ",quote = FALSE )
write.table(info_split, file =  paste0(dir.file,"/full_split.ped"), row.names = FALSE, col.names = FALSE,sep = ",",quote = FALSE)


write.table(info, file =  paste0(dir.file,"/train.ped"), row.names = FALSE, col.names = FALSE,sep = " ",quote = FALSE )
write.table(info_test, file =  paste0(dir.file,"/test.ped"), row.names = FALSE, col.names = FALSE,sep = " ",quote = FALSE)


write.table(info_50k, file =  paste0(dir.file,"/train_50.ped"),row.names = FALSE, col.names = FALSE,sep = " ",quote = FALSE )
write.table(info_test_50k, file =  paste0(dir.file,"/50_test.ped"), row.names = FALSE, col.names = FALSE,sep = " ",quote = FALSE)
...
system(paste0("plink --file " ,dir.file,"/train --make-bed --out " ,dir.file,"/beforeQC"))

system(paste0("plink --file ",dir.file,"/train_50 --make-bed --out " ,dir.file,"/beforeQC_50"))
